...
- `model-router.ts` — 多 profile（`config/models.json`，支持 `${ENV}`）。**思考模式**：`thinking: true` 时 temperature 失效，经 `extra_body` 传递；流式 usage 只在循环外一次性 `+=`；`lastUsage` 供 assistant 事件携带 token；**`addProfile(key, {...})` 运行时添加模型 profile（立即生效，`getProfileRaw` 供持久化写回 config/models.json）**
...
/config                  模型/温度/max-tokens/iterations/thinking/skill-evo/add-model/reset（持久化 data/runtime-config.json + config/models.json）
/sessions                浏览会话
/switch <序号>           切换
