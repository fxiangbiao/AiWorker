...
- `src/tools/ask-channel.ts` — **ask_user 提问通道**：`AskProvider` 全局分发（对齐 confirm-channel），CLI 走 stdin，HTTP 走 SSE `ask_user` 挂起 + POST `/api/v1/ask`；`ask_user` 已加入只读白名单（`PermissionModel.READONLY_TOOLS`）
