<script lang="ts">
  ...
  function startListDrag(e: PointerEvent) {
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const total = panel.parentElement?.clientWidth ?? 800;
...
<div class="diff-body">
  <div class="diff-list" style:width={listWidth ? `${listWidth * 100}%` : "20%"}>
    ...
    {:else}
      {#each sessions as s}
        <div class="dl-session">{s.sessionId.slice(0, 8)}...</div>
        {#each s.files as f}
          <div
            class="dl-file"
            ...
            tabindex="0"
          >
            <span class="dl-name">{basename(f.path)}</span>
...
<style>
  ...
  .dl-session { font-size: 10px; color: var(--dim); margin: 8px 0 4px; }
  .dl-file {
    display: flex;
    align-items: center;
    gap: 6px;
