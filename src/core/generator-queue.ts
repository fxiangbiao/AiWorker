/**
 * 生成任务队列（Sprint 35 补丁）— P1 异步生成
 * 入队即返 jobId（连接零阻塞）；后台调 factory.generate；状态流转 + gen/* 事件广播
...
export class GeneratorQueue {
  ...
  submit(spec: AppSpec): string {
    ...
    this.jobs.set(id, { id, spec, status: "queued", createdAt: Date.now() });
    this.queue.push(id);
  ...
  private async run(job: GenJob): Promise<void> {
    ...
    try {
      const result = await factory.generate(job.spec, (step, index, total, detail) => {
        job.step = step;
