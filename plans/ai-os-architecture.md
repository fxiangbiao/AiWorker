...
**现状评估**：浅色主题、13px 紧凑字号、统一 CSS 变量、实用主义工具风——底子好；不足：无暗色模式、信息架构偏"聊天工具"、图标/按钮样式不统一（emoji 实体混用 20+ 处）、滚动条全隐藏（Windows 用户拖不了）、无 `:focus-visible`、空状态空白、响应式简陋（768px 以下直接隐藏侧栏）。

**与 4.7 的关系**：左侧 OS 导航 = **高频工作区入口**（对话/应用/任务/记忆）；SystemPanel 弹窗 = **深度管理**（配置/日志/进化/设备）——互补不冲突。

**🟢 低成本高价值（Sprint 34 前置，约 1-2 天）**：
1. **暗色模式**：CSS 变量已集中在 `:root`，加 `[data-theme="dark"]` 变量覆盖 + 顶栏开关 + localStorage 记忆
2. **统一图标**：lucide（tree-shakable，Svelte 友好）替换 emoji/HTML 实体图标
3. **细滚动条**：6px 窄版（hover 加深）替代全隐藏，解决"不知道能滚"
4. **可访问性**：全局 `:focus-visible` 样式 + 现有 `role="button"` 元素补齐键盘导航
5. **空状态启动台**：新对话空页放 OS 快捷入口（新对话 / 生成应用 / 打开进程 / 语音输入）——立起 AI OS 门面

**🟡 中成本（随 Sprint 34/35 落地）**：
6. **左侧栏 OS 导航**：一级入口「对话 / 应用 / 任务 / 记忆 / 设置」，会话列表降为"对话"子视图——从"聊天工具"到"OS 控制台"视觉转变的关键一步（配合 4.7）
7. **顶栏整合**：logo + 全局状态（在线/模型/**预算占用条**）+ 模式/Agent + 系统入口；StatusBar 的 tokens/cost 上移或并入进程视图
8. **消息流过程可视化**：ToolCard 加耗时/预算徽标、工具调用折叠为内联轨迹

**🔴 高成本（1.0 整合阶段）**：
9. **主题 token 文档化**：色板/间距/字号/圆角/阴影层级成文档，暗色+浅色双主题打磨
10. **多窗口布局适配**：AppHostLayer 窄屏降级策略、应用窗口主题继承

**原则**：UI 优化**不是独立 Sprint，渗透进各 Sprint**；克制工具感优先，不做营销感动画。
