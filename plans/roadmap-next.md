...
| # | 事项 | 报告差距 | 关键设计取舍 |
|---|---|---|---|
| P1-1 | **后台子智能体**：`spawn/fork` + `send_message` / `interrupt_agent` / `list_agents` | 4.3-#9 并行长程弱 | 先做"可续接的子会话"（复用 `session_events` + `wk-` 隔离），不做进程池；**前置：P1-4 权限记忆**（子智能体无确认通道，只能靠预先授予的规则） |
