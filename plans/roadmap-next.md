...
> 基线：AiWorker 1.4.0 / commit `8dad3aa` / 测试 824 全绿
...
## 三、P1 — 长程与工具纵深（Sprint 49–51）

| # | 事项 | 报告差距 | 关键设计取舍 |
|---|---|---|---|
| P1-1 | **后台子智能体**：`spawn/fork` + `send_message` / `interrupt_agent` / `list_agents` | 4.3-#9 并行长程弱 | 先做"可续接的子会话"（复用 `session_events` + `wk-` 隔离），不做进程池 |
| P1-2 | **目标驱动（goal）**：持久目标 + 轮次驱动 + 事件溯源 | 4.3-#9 | 复用事件日志与 TurnLog，不引入新存储 |
| P1-3 | 工具面扩展：LSP（本地 TS/Python）、浏览器自动化、Notebook 编辑、图像理解 | 4.3-#10 工具面窄 | 每项独立可关；能力声明进入工具白名单体系 |
| P1-4 | 权限记忆：allow-always（会话级/项目级）+ 一键撤销 + `/permissions` 可视化 | 4.1-#3 审批无记忆 | 规则持久化到 `config/permissions.json`（复用 S47 schema） |
| P1-5 | 压缩与成本：压缩阈值可调 + 工具结果剪枝 + 本地成本估算（仅展示） | 4.3-#14 策略单一 | 不做计费，只做"看得见" |
| P1-6 | 会话体验：搜索/分组/归档、跨会话引用、突破 50 条上限 | 4.4-#15 | 纯前端 + `/sessions?limit` 已有基础 |
| P1-7 | 沙箱增强：fs 工具的写根与受保护路径统一走策略层（当前仅命令层） | 4.1-#1 | 与 S47 的 `allowWriteDirs` 对齐 |

---

## 四、P2 — 场景扩张与生态（Sprint 52+）
